import { Aes128 } from '@discoursegroup/commons-js';
import { RRPayload, Transformer } from '@discoursegroup/relayrabbit-commons-js';

/**
 * This transformer is used to encrypt an array buffer using AES 128.
 * It is used to encrypt the payload within the pipe call chain.
 */
export class ArrayBufferToEncryptAes128Transformer implements Transformer {
  private aesInstance: Aes128;

  /**
   * Constructor takes the key to be used for encryption.
   * The key is used to initialize the AES 128 instance. It is not stored or retained by the code in any way.
   * The key is not shared with the remote service or any other party.
   * @param {string} key - to be exactly 128 bits long.
   */
  constructor(key: string) {
    this.aesInstance = new Aes128(key);
  }
